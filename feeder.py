...
class Feeder_mediapipe(torch.utils.data.Dataset):
    """ Feeder for skeleton-based action recognition in mediapipe-skeleton dataset
    Arguments:
    ...
    def __init__(self,
        ...
        self.num_hand_in = num_hand_in
        self.num_hand_out = num_hand_out
    ...
    def __getitem__(self, index):
        ...
        frameNum = 1
        # fill data_numpy
        data_numpy = np.zeros((self.C, self.T, self.V, self.num_hand_in))
        for frame_info in video_info['data']:
            frame_index = frame_info['frame_index']
            for m, skeleton_info in enumerate(frame_info["skeleton"]):
                if m >= self.num_hand_in:
                    break
                # pose = skeleton_info['pose']
                # score = skeleton_info['score']
                # print("pose", pose)
                # print("score", score)
                # print("pose02", pose[0::2])
                # print("pose12", pose[1::2])
                # data_numpy[0, frame_index, :, m] = pose[0::2]
                # data_numpy[1, frame_index, :, m] = pose[1::2]
                # data_numpy[2, frame_index, :, m] = score
                # print("data numpy", data_numpy)
                data_numpy[0, frame_index, :, m] = skeleton_info[0::3]
                data_numpy[1, frame_index, :, m] = skeleton_info[1::3]
                data_numpy[2, frame_index, :, m] = skeleton_info[2::3]
            frameNum = frameNum + 1
        ...
        # data_numpy = data_numpy[:, :, :, 0:self.num_hand_out]
    ...
    def calculate_recall_precision(self, score):
        assert (all(self.label >= 0))
        return calculate_recall_precision(self.label, score)